...
HuffmanTree::HuffmanTree(const HuffmanTree& other)
{
	...
	{
		m_freqTable[i] = new HuffmanNode;
		m_freqTable[i] = other.m_freqTable[i];
		m_freqTable[i]->color = other.m_freqTable[i]->color;
		m_freqTable[i]->freq = other.m_freqTable[i]->freq;
	}
}
...
std::string HuffmanTree::encode(std::vector<unsigned char> buffer)
{
	...
	for (int i = 0; i < 256; i++)
	{
		if (m_freqTable[i]->freq) pq.push(m_freqTable[i]);
	}
...
std::vector<unsigned char> HuffmanTree::decode(std::fstream& stream)
{
	...
	for (int i = 0; i < text.size() - 1; i++)
	{
		...
		if (i == text.size() - 2) path = path.substr(0, 8 - count);
...
inline void HuffmanTree::build_tree(std::string& path, unsigned char a_code)
{
	...
	for (int i = 0; i < path.size(); i++)
	{
		if (path[i] == '0')
		{
			if (current->left == nullptr) current->left = new HuffmanNode;
			current = current->left;
		}
		else if (path[i] == '1')
		{
			if (current->right == nullptr) current->right = new HuffmanNode;
